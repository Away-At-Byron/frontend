...
export function ContactDetail({
  ...
  return (
    ...
    >
      ...
      {tab === "history" && (
        <PlaceholderTab icon="Calendar" label="Guest History" />
      )}
      {tab === "communication" && (
        <CommunicationTab
          form={form}
...
function PlaceholderTab({ icon, label }: { icon: IconName; label: string }) {
  return (
    <Card pad={48}>
      <div
        style={{
      ...
      >
        <IconButton size={56} variant="shell" title={label}>
        ...
        <div style={{ fontSize: 13.5 }}>Nothing to show yet.</div>
      </div>
    </Card>
  );
}
