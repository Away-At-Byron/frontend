...
import {
  NewRoomAmenityModal,
  EditRoomAmenityModal,
} from "./room-amenity-modal"
...
function sortByName(rows: RoomAmenityRow[]): RoomAmenityRow[] {
  return [...rows].sort((a, b) => a.name.localeCompare(b.name))
}

export function RoomAmenityManagement({
  initialAmenities,
}: {
  ...
  if (initialAmenities !== syncedFrom) {
    setSyncedFrom(initialAmenities)
        ...
        <div style={{ overflowX: "auto" }}>
          <div style={{ minWidth: 820 }}>
            ...
            {filtered.length === 0 ? (
              ...
              >
                {rows.length === 0
                  ? "No room amenities yet. Add the first one."
                  : "No room amenities match this search."}
              </div>
            ) : (
              filtered.map((r, i) => (
                <div
                ...
                >
                  ...
                  <span>
                    <Pill tone={r.usageCount > 0 ? "info" : "neutral"}>
                      {r.usageCount} room{r.usageCount === 1 ? "" : "s"}
                    </Pill>
                  </span>
